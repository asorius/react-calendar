...
// const circle = () => {
//   // return <circle r={5} cx={0} cy={0}></circle>;
//   return ;
// };
export default function Bookings({ children }: { children: React.ReactNode }) {
  const body = React.useRef<SVGPathElement>(null);
  const tip = React.useRef<SVGPathElement>(null);
  const svg = React.useRef<SVGSVGElement>(null);

  if (body.current && tip.current) {
    // console.log(body.current.getTotalLength());
    // console.log(tip.current.getTotalLength());
    const path = body.current;
    const val = { distance: 0 };
    // Create a tween
    path.setAttribute('stroke-dasharray', path.getTotalLength().toString());
    path.setAttribute('stroke-dashoffset', path.getTotalLength().toString());
    gsap.to(path, {
      strokeDashoffset: 0,
